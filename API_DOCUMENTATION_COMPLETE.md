# Currency API - Полная документация
...
curl -X POST http://localhost:8000/api/device/register \
  ...
  -d '{"device_id": "my-device-123", "platform": "iOS"}'
```
...
API не требует аутентификации для базовых операций. Для отслеживания устройств рекомендуется передавать `device_id` в параметрах запроса или заголовке `X-Device-ID`.
...
(async () => {
    try {
        // Регистрация устройства
        await registerDevice('my-device-123', {
            device_name: 'iPhone 13',
            platform: 'iOS',
            app_version: '1.0.0'
        });
        ...
        console.log('EUR курс:', rates.EUR.rate);
...
try {
    ...
    // Получение курсов
    $rates = getRates();
    echo "EUR курс: " . $rates['EUR']['rate'] . "\n";
    
    // Конвертация
    $converted = convertCurrency(100, 'USD', 'EUR');
    echo "100 USD = $converted EUR\n";
} catch (Exception $e) {
...
class CurrencyAPI:
    ...
    def get_rates(self, base='USD'):
        """Получить все курсы валют"""
        url = f'{self.base_url}/rates?base={base}'
...
try:
    # Регистрация устройства
    api.register_device('my-device-123', 
                       device_name='iPhone 13',
                       platform='iOS',
                       app_version='1.0.0')
    ...
    # Получение курсов
    rates = api.get_rates()
    print(f"EUR курс: {rates['EUR']['rate']}")
    
    # Конвертация
    converted = api.convert(100, 'USD', 'EUR')
...
curl "http://localhost:8000/api/device/info?device_id=my-device-123"
...
Всего поддерживается ~166 валют.
...
- **Интерактивная документация:** http://localhost:8000/api-docs.html
- **OpenAPI спецификация:** swagger.yaml
- **Примеры использования:** api_examples.md
- **Админ панель:** http://localhost:8000/admin/ (пароль: admin123)
